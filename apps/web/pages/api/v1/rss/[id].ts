...
export default async function handler(
  ...
  if (!Number.isInteger(rssId) || rssId <= 0) {
    return res.status(400).json({ response: "Invalid RSS subscription id." });
  }
  ...
  if (req.method === "PUT") {
    ...
    const dataValidation = UpdateRssSubscriptionSchema.safeParse(req.body);
    ...
    const response = await prisma.rssSubscription.update({
      ...
      data: {
        ...
        lastBuildDate: url !== rssSubscription.url ? null : undefined,
      },
    });
