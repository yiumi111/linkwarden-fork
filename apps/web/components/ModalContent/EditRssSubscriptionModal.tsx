...
type Props = {
  onClose: Function;
  rssSubscription: RssSubscription & { collection: { name: string } };
};
...
export default function EditRssSubscriptionModal({
  ...
  const submit = async () => {
    ...
    await editRssSubscription.mutateAsync(
      { id: rssSubscription.id, ...form },
      {
        onSettled: (_, error) => {
  ...
  return (
    <Modal toggleModal={onClose}>
      <>
        <p className="text-xl font-thin">{t("edit_rss_subscription")}</p>
