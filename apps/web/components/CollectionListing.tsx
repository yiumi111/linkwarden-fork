...
const CollectionListing = () => {
  ...
  const initialTree = useMemo(() => {
    ...
    const newTree = buildCollectionTree(collections, user?.collectionOrder);

    if (tree) {
      const expandedIds = Object.keys(tree.items).filter(
        (id) =>
          id !== "root" && tree.items[id]?.isExpanded === true
      );
      return applyExpandedState(newTree, expandedIds);
    }

    const activeCollectionId = Number(router.asPath.split("/collections/")[1]);
    if (activeCollectionId) {
      const parentIds = getExpandedParentIds(
        newTree.items as Record<string, CollectionTreeItem>,
        activeCollectionId
      );
      return applyExpandedState(newTree, parentIds);
    }

    return newTree;
  }, [collections, user, tree, router.asPath]);
  ...
  const onExpand = (collectionId: ItemId) => {
    setTree((currentTree) =>
      mutateTree(currentTree!, collectionId, { isExpanded: true })
    );
  };

  const onCollapse = (collectionId: ItemId) => {
    setTree((currentTree) =>
      mutateTree(currentTree as TreeData, collectionId, {
        isExpanded: false,
      })
    );
  };
  ...
  const onDragEnd = async (
    ...
    const newTree = applyReorderToTree(
      tree,
      movedCollectionId,
      source,
      destination
